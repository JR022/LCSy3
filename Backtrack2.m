function Backtrack2(array)
%Scales output coordinates
...
currentRow = 960; currentColumn = 1000; %Stores the current position of the path
...
for point = 1:(length(history)-1)/2
    historyString = historyString + sprintf("(%.2f,%.2f)--",history(2*point-1)/10,history(2*point)/10);
end
historyString = historyString + sprintf("(%.2f,%.2f)",history(2*point+1)/10,history(2*point+2)/10);
